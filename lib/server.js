  ...
  cds.on('served', () => {
    ...
    ds.prepend(() => {
      ds.before('deploy', async ({ data }) => {
        ...
        // step 2: evaluate config: which service to be connected to which service manager (target container)
        // Priority: dynamic config (config table) > static config (cds.env.requires entries with kind 'hana-synonyms')
        // also compile list of all used target db service managers
        const allConfig = await getResolvedConfig()
        let target_containers = {};
        for (const { srv, target, source } of allConfig) {
          if (srv in cds_services) {
            cds_services[srv] = target ?? null
            if (target) target_containers[target] = null
            LOG.info('===== before deploy - resolved ' + srv + ' -> ' + cds_services[srv] + ' (' + source + ')')
          }
        }
        // example: cds_services: { 'sap.capire.bookshop.data' : 'syn-prov-1-mt-db', ... }
