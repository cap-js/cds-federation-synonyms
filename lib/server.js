  ...
  cds.on('served', () => {
    ...
    ds.prepend(() => {
      ds.before('deploy', async ({ data }) => {
        ...
        // Priority: static config (cds.env.requires entries with kind 'hana-synonyms') > config table
        const allConfig = await getResolvedConfig()
        for (const { srv, target, source } of allConfig) {
          if (srv in cds_services) {
            // only apply the effective (static-wins) entry; skip duplicate dynamic entries for overridden services
            if (source.startsWith('static') || !allConfig.some(e => e.srv === srv && e.source.startsWith('static'))) {
