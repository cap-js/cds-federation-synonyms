  ...
  cds.on('served', () => {
    ...
    ds.prepend(() => {
      ds.before('deploy', async ({ data }) => {
        ...
        let cds_services = Object.fromEntries(ext_services.map(([n]) => [n, undefined]));
        // example: cds_services: { 'sap.capire.bookshop.data' : undefined, ... }
        ...
        for (const { srv, target, source } of allConfig) {
          if (srv in cds_services && cds_services[srv] === undefined) {
            // first match wins; dynamic entries come first in allConfig, so dynamic overrides static
