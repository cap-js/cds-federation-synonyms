...
function buildPlugin() {
  ...
  cds.compile.to.hana = function (csn, options, ...etc) {
    ...
    for (const n in csn.definitions) {
      ...
      // for exported entity:
      // add object privileges to the service's roles
      ...
      if (srv_i) {
        ...
        if (!srv_i.quotedNames) {
          ...
          // csn_small and qgrants only needed for quoted names
        }
        else {
          syn_name         = n + '#syn';
    ...
    if (Object.keys(csn_small.definitions).length > 0) {
      const optQ = { ...options, sqlMapping: 'quoted' };
      hdbtable_quoted = compile.to.hdi.migration(csn_small, optQ, null).definitions
    }
    ...
    for(let result of hdiResult) {
      ...
      // remove .hdbindex and .hdbconstraint files for imported entities,
      //  as they are not needed for mock tables and would cause build errors
      const indexOrConstraint =
        (file.file.endsWith('.hdbindex') || file.file.endsWith('.hdbconstraint')) &&
        di_services_imp.some(service => file.file.startsWith(service.name))
      if (indexOrConstraint) continue
